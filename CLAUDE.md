...
**`CaseStudyHero`** — `noWrapTitle` prop stacks title above tagline/meta (vs. default side-by-side `.left` + `.metaStrip` row) via `.titleNoWrap` (`white-space: nowrap` desktop). At `≤768px` this is overridden to `white-space: normal; overflow-wrap: break-word` so long titles (e.g. Trust Calibration's) wrap to multiple lines instead of forcing horizontal overflow.
...
**`TrustCalibrationFlow`** — `'use client'` component rendering the 5-step calibration workflow diagram (Trust Calibration case study). Steps are rect nodes (`user-action` / `system`) or a rotated-square `diamond` (`decision`). Desktop: horizontal row, connectors are horizontal lines (`flex: 1`) with a right-pointing arrowhead, all shapes vertically centered via a shared `.shapeArea` (148px). Mobile (`≤768px`): column layout, connectors become vertical lines with a down-pointing arrowhead — connector needs its own `flex: 0 0 auto` in the mobile query (a bare `flex: 1` inherited from the desktop rule collapses the line's height to 0 in a column flex container, leaving only the arrowhead visible). The diamond is a rotated square, so its rendered bounding box is ~1.41× its `width`/`height` — at the shared mobile `.shapeArea` height (100px) a 100px diamond overflows into the arrow above and the annotation below. Fixed via a `styles.shapeAreaDecision` modifier class applied only to the decision node (`step.type === 'decision'` in `TrustCalibrationFlow.js`), giving just that node a taller band (140px) with a smaller diamond (90px) — the other four nodes keep the normal tight 100px band.
...
- `src/lib/analytics.js` — `initPostHog()` (idempotent, `capture_pageview: false`) + `capture(event, props)` safe wrapper (lazily inits, no-ops without key). All event calls go through `capture`, never `posthog.capture` directly. Init also exposes `window.posthog` (npm bundle has no console handle otherwise) and checks for a `?no_track=1` query param — visiting any page with it calls `posthog.opt_out_capturing()`, permanently opting that browser out (used to exclude Mikael's own devices from analytics).
...
**Gotcha — mobile horizontal scroll:** `html`/`body` in `globals.css` carry `overflow-x: hidden` as a site-wide safeguard. Elements wider than the viewport at small breakpoints (long non-wrapping titles, fixed-width tables) will get silently clipped rather than pushing out a page-level horizontal scrollbar — so if content goes missing on mobile, check for overflow before assuming it's a display bug. For content that's *legitimately* wider than mobile viewports (e.g. the Stakeholders comparison table on `/trust-calibration`), wrap it in a card with `overflow-x: auto` and give the inner element (e.g. `table`) a `min-width` so it scrolls horizontally within its own bounds instead of being clipped or squeezing its content unreadably — see `.cardWrap` / `.table` in `trust-calibration.module.css`.
